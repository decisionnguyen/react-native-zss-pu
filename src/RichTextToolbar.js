...
import {ListView, View, TouchableOpacity, Image, StyleSheet, ScrollView} from 'react-native';
    ...
    return (
      ...
      >
        {icon ? <Image source={icon}
                       style={{tintColor: selected ? this.props.selectedIconTint : this.props.iconTint}}/> : null}
      </TouchableOpacity>
    );
  }
    ...
    return (
      ...
      >
        <ScrollView horizontal={true}>
          {
            this.state.ds.map(row => {
              return this._renderAction(row.action, row.selected)
            })
          }
        </ScrollView>
      </View>
